#pragma once
#include "DAG.h"
class IPPTS_wbar : public scheduler {
public:
	double duration;
	int PCM(int, int);
	//double rankPCM(int);
...
int IPPTS_wbar::PCM(int ni, int pk)
{
...
/*double IPPTS_wbar::rankPCM(int ni)
{
...
double IPPTS_wbar::Prank(int ni)
{
	double value = 0.0, total = 0.0;
	double od = 1.0;
	for (int k = 0; k < num_processor; k++) {
...
void IPPTS_wbar::CreatDAG()
{
	DAG::DAG_generate();
}

void IPPTS_wbar::scheduling() {
	...
	File <<"wbar" << "," << DAG::num_task << "," << DAG::num_processor << "," << DAG::task[(DAG::num_task - 1)].AFT << "," << duration << endl;
	File.close();
