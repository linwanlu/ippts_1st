#pragma once
#include "DAG.h"
class IPPTS_ndwpe : public scheduler {
public:
	double duration;
	int PCM(int, int);
	//double rankPCM(int);
...
int IPPTS_ndwpe::PCM(int ni, int pk)
{
...
double IPPTS_ndwpe::Prank(int ni)
{
	double value = 0.0, total = 0.0;
	double od = 1.0;
	for (int k = 0; k < num_processor; k++) {
...
void IPPTS_ndwpe::CreatDAG()
{
//	task::rand_processor();
	DAG::DAG_generate();
}

void IPPTS_ndwpe::scheduling() {
	...
	File <<"ippts_ndwpe" << "," << DAG::num_task << "," << DAG::num_processor << "," << DAG::task[(DAG::num_task - 1)].AFT << "," << duration << endl;
	File.close();
