#pragma once
#include "DAG.h"
class IPPTS_minlhead : public scheduler {
public:
	double duration;
	int PCM(int, int);
	//double rankPCM(int);
...
int IPPTS_minlhead::PCM(int ni, int pk)
{
	...
	for (int i = 0; i < num_task; i++) {
		if (DAG::DAG[ni][i] != 0) {
			for (int k = 0; k < num_processor; k++) {
				if (pk == k) {
					if (minPath == 0)
						{ minPath = DAG::task[i].pcm[k] + DAG::task[i].ComputationCosts[k]; }
					else 
					{ minPath = min(minPath, DAG::task[i].pcm[k] + DAG::task[i].ComputationCosts[k]); }
				}
				else  {
					if (minPath == 0) 
						{ minPath = DAG::task[i].pcm[k] + DAG::task[i].ComputationCosts[k] + DAG::DAG[ni][i]; }
					else 
					{ minPath = min(minPath, DAG::task[i].pcm[k] + DAG::task[i].ComputationCosts[k] + DAG::DAG[ni][i]); }
				}
			}
...
/*double IPPTS_minlhead::rankPCM(int ni)
{
...
double IPPTS_minlhead::Prank(int ni)
{
	double value = 0.0, total = 0.0;
	double od = 1.0, min=100.0;
	for (int k = 0; k < num_processor; k++) {
		total += PCM(ni, k);
	}
	for (int k = 0; k < num_processor; k++) {
		if(DAG::task[ni].ComputationCosts[k]<min)
			min = DAG::task[ni].ComputationCosts[k];
	}
	total = (total/num_processor) + min;
...
void IPPTS_minlhead::sort_Prank()
{
	double temp0;
	double temp1;
...
void IPPTS_minlhead::CreatDAG()
{
	//task::rand_processor();
	DAG::DAG_generate();
}

void IPPTS_minlhead::scheduling() {
	...
	for (int i = 0; i < num_task; i++) {
		...
		for (int j = 0; j < num_processor; j++) {
			...
			DAG::task[tasknow].Lhead[j] = PCM(tasknow, j) + EFT(tasknow, j);
	...
	File <<"minlhead" << "," << DAG::num_task << "," << DAG::num_processor << "," << DAG::task[(DAG::num_task - 1)].AFT << "," << duration << endl;
	File.close();
