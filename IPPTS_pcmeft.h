#pragma once
#include "DAG.h"
class IPPTS_pe : public scheduler {
public:
	double duration;
	int PCM(int, int);
	//double rankPCM(int);
...
int IPPTS_pe::PCM(int ni, int pk)
{
...
void IPPTS_pe::CreatDAG()
{
//	task::rand_processor();
	DAG::DAG_generate();
}

void IPPTS_pe::scheduling() {
	...
	File <<"ippts_pe" << "," << DAG::num_task << "," << DAG::num_processor << "," << DAG::task[(DAG::num_task - 1)].AFT << "," << duration << endl;
	File.close();
