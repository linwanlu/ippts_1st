...
class IPPTS_wbarlhead : public scheduler {
public:
	double duration;
	int PCM(int, int);
	//double rankPCM(int);
...
int IPPTS_wbarlhead::PCM(int ni, int pk)
{
...
/*double IPPTS_wbarlhead::rankPCM(int ni)
{
...
double IPPTS_wbarlhead::Prank(int ni)
{
	double value = 0.0, total = 0.0;
	double od = 1.0;
	for (int k = 0; k < num_processor; k++) {
	...
	total = total/num_processor + avg_ComputationCost[ni];
...
void IPPTS_wbarlhead::sort_Prank()
{
	double temp0;
	double temp1;
...
void IPPTS_wbarlhead::CreatDAG()
{
	//task::rand_processor();
	DAG::DAG_generate();
}

void IPPTS_wbarlhead::scheduling() {
	...
	File <<"wbarlhead" << "," << DAG::num_task << "," << DAG::num_processor << "," << DAG::task[(DAG::num_task - 1)].AFT << "," << duration << endl;
	File.close();
